...
class RotatingVerification {
  ...
  initHandler(
    handlerVue: InstanceType<typeof RotatingVerificationCom> | undefined
  ) {
  ...
  request(data: RequestData | undefined): Promise<ResponseData> {
    return new Promise(async (solve) => {
      ...
      const imgRes = await api.getVerifyImg()

      if (imgRes == undefined || imgRes.data == undefined) {
        solve({
          success: false,
          session: ''
        })
...
export { rotatingVerification as RobotVerification }
