...
const currentActivity = ref(
  volunteerActivity.value.find((item) => item.id === Number(route.params.id))
)
const {
  activitySignupInfo,
  handleApply: storeHandleApply,
  handleCancelApply: storeHandleCancelApply
} = applyStore
// 报名
const handleApply = () => {
  ...
  <div v-if="currentActivity">
    ...
    <div class="activity-info">
      <div class="activity-title">
        ...
        <span>
          已报名:<span style="color: darkmagenta">{{
            activitySignupInfo[currentActivity.id]?.recruitedNum || 0
          }}</span
          >/{{ currentActivity.num }}
        </span>
      </div>
      <p>{{ currentActivity.startTime }}--{{ currentActivity.deadline }}</p>
      <p style="color: darkmagenta; font-size: small">
        截止报名时间:{{ currentActivity.deadline }}
      </p>
      <strong>活动类型:</strong>
    ...
    <div class="apply">
      <!-- 报名 -->
      <van-button
        v-if="!activitySignupInfo[currentActivity.id]?.isApplied"
        type="primary"
        width="100%"
        @click="handleApply"
        >点击报名</van-button
      >
      <!-- 取消报名 -->
      <van-button v-else type="danger" width="100%" @click="handleCancelApply"
        >取消报名</van-button
      >
    </div>
  </div>
